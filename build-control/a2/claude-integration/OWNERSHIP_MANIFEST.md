...
# OWNERSHIP_MANIFEST — A2-CLAUDE-INTEGRATION
...
**Committed and frozen:** `contracts/CONTRACT_PLUGIN_001.md`, `CONTRACT_PLUGIN_002.md`, `CONTRACT_CLI_001.md`

**Planned, created only on wave authorization:** `.claude-plugin/plugin.json`, `hooks/hooks.json`, `skills/*/SKILL.md`, `agents/*.md`, plugin-root `settings.json`, `src/entry/**`, `bin/receipts`, `schemas/hook-request.schema.json`, `schemas/hook-decision.schema.json`, `schemas/cli-envelope.schema.json`, recorded hook fixtures, `build-control/a2/claude-integration/**`, and the tests for all of them.

`agents/receipts-reviewer.md` is yours to write; its read-only tool list is **A2-TRUST's requirement** and needs their approval.

## FILES / DIRECTORIES CONSUMED

`contracts/CONTRACT_POLICY_002.md`, `CONTRACT_CORE_001.md`, `CONTRACT_CORE_002.md`, `CONTRACT_CORE_003.md`, `CONTRACT_CONFIG_001.md`, `CONTRACT_CONFIG_002.md`, `CONTRACT_OVERRIDE_001.md`, `CONTRACT_REVIEW_003.md`, and the orchestration files listed as CONSUMED. Read and obey; never edit.

## FILES / DIRECTORIES REFERENCE-ONLY

`contracts/CONTRACT_POLICY_001.md`, `CONTRACT_EVIDENCE_001.md`, `CONTRACT_RUNNER_001.md`, `CONTRACT_RUNNER_002.md`, `CONTRACT_REVIEW_001.md`, `CONTRACT_REVIEW_002.md`, `CONTRACT_CONFIG_003.md`, the remaining orchestration files, and the superseded 8-manager package if it is committed to the repository (historical only, never authority).

## FILES / DIRECTORIES FORBIDDEN TO MODIFY

`Receipts_Final_Architecture.md`; `orchestration/**`; `architecture-decisions/**`; `contracts/CONTRACT_INDEX.md`; `schemas/SCHEMA_PLAN.md`; `A2_CONSOLIDATION_DECISION.md`; `A2_OWNERSHIP_REMAP.md`; every contract you do not own; `src/core/**`; `src/adapters/git/**`; `src/adapters/runner/**`; `src/adapters/providers/**`; `eval/**`; `docs/**`; `README.md`; and every foreign schema.
...
- Branch: `a3/claude-integration/<task-id>`, one worktree per active implementation task
