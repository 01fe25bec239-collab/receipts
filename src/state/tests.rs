...
fn t09_lower_unsupported_version_fails() {
    ...
    // Database initialized at version 8 by an eight-migration chain.
    let v8_chain = chain_with(&[probe_migration(8, "probe_v8")]);
    drop(
        SqliteStateRepository::open_with_migrations(tmp.db_path(), &v8_chain)
            .expect("bootstrap at version 8"),
    );
    // Opening against a chain supporting version 9 must fail closed.
    let v9_chain = chain_with(&[
        probe_migration(8, "probe_v8"),
        probe_migration(9, "probe_v9"),
    ]);
    let error = SqliteStateRepository::open_with_migrations(tmp.db_path(), &v9_chain)
    ...
    let repo = SqliteStateRepository::open_with_migrations(tmp.db_path(), &v8_chain)
