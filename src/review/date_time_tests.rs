use super::{ReviewDateTimeError, ReviewDateTimeV1};
...
fn mandatory_valid_examples() {
    for input in [
        ...
        assert_eq!(ReviewDateTimeV1::try_new(input).unwrap().as_str(), input);
    }
}
...
fn mandatory_invalid_examples() {
    for input in [
        ...
        assert_eq!(
            ReviewDateTimeV1::try_new(input),
            Err(ReviewDateTimeError),
            "{input:?}"
        );
    }
}
...
fn calendar_boundaries() {
    for (date, accepted) in [
        ...
        assert_eq!(
            ReviewDateTimeV1::try_new(format!("{date}T00:00:00Z")).is_ok(),
            accepted,
            "{date}"
        );
    }
    for (month, last_day) in [
        ...
        for day in 0..=32 {
            ...
            assert_eq!(
                ReviewDateTimeV1::try_new(input.clone()).is_ok(),
...
fn representations_remain_exact_and_lexically_distinct() {
    ...
    let values: Vec<_> = inputs
        .iter()
        .map(|input| {
            let value = ReviewDateTimeV1::try_new(*input).unwrap();
            assert_eq!(value.as_str().as_bytes(), input.as_bytes());
            assert_eq!(value, ReviewDateTimeV1::try_new(input.to_string()).unwrap());
            value
        })
        .collect();
...
fn fractional_precision_has_no_artificial_limit() {
    for length in [1, 3, 9, 18, 10_000] {
        ...
        let value = ReviewDateTimeV1::try_new(input.clone()).unwrap();
...
fn non_ascii_and_truncated_inputs_reject_without_panicking() {
    ...
    for lookalike in ["２", "٢", "−", "：", "🦀", "é", "\u{0301}"] {
        ...
        for index in 0..=valid.len() {
            ...
            assert_eq!(ReviewDateTimeV1::try_new(input), Err(ReviewDateTimeError));
            if index < valid.len() {
                let mut input = valid.to_owned();
    ...
    for end in 0..valid.len() {
        assert_eq!(
            ReviewDateTimeV1::try_new(&valid[..end]),
            Err(ReviewDateTimeError)
        );
    }
}
